...
function renderAssets() {
  ...
  const rows = state.assets.filter((asset) => `${asset.tag_code} ${asset.name}`.toLowerCase().includes(query));
  const actions = (asset) => state.user?.role === 'admin' ? `<button class="icon-button" data-edit-asset="${asset.id}" title="Edit asset">Edit</button><button class="icon-button danger" data-delete-asset="${asset.id}" title="Delete asset">Delete</button>` : state.user?.role === 'staff' ? `<button class="icon-button" data-edit-asset="${asset.id}" title="Update operational fields">Update</button>` : state.user?.role === 'manager' && asset.condition_status !== 'retired' ? `<button class="icon-button" data-approve-retirement="${asset.id}" title="Approve retirement">Approve retirement</button>` : '<span class="muted">View only</span>';
  $('#assets-body').innerHTML = rows.length ? rows.map((asset) => `${'<tr>'}<td class="mono">${escapeHtml(asset.tag_code)}</td><td>${escapeHtml(asset.name)}</td><td>${escapeHtml(asset.category_name)}</td><td>${escapeHtml(asset.location_name || '-')}</td><td><span class="status ${asset.condition_status}">${escapeHtml(asset.condition_status.replace('_', ' '))}</span></td>${state.user?.role === 'staff' ? '' : `<td>${asset.current_value === null ? '—' : Number(asset.current_value).toFixed(2)}</td>`}<td>${escapeHtml(asset.assigned_name || '-')}</td><td class="actions">${actions(asset)}</td></tr>`).join('') : '<tr><td class="empty-cell" colspan="8">No assets registered yet.</td></tr>';
}

function renderMaintenance() {
  $('#maintenance-body').innerHTML = state.maintenance.length ? state.maintenance.map((item) => { const actions = state.user?.role === 'admin' ? `<button class="icon-button" data-edit-maintenance="${item.id}">Edit</button><button class="icon-button danger" data-delete-maintenance="${item.id}">Delete</button>` : state.user?.role === 'staff' && item.status !== 'completed' ? `<button class="icon-button" data-complete-maintenance="${item.id}">Mark complete</button>` : '<span class="muted">View only</span>'; return `<tr><td class="mono">${escapeHtml(item.tag_code)}</td><td>${escapeHtml(item.asset_name)}</td><td>${escapeHtml(item.task)}</td><td>${dateText(item.scheduled_date)}</td><td><span class="status ${item.status}">${escapeHtml(item.status.replace('_', ' '))}</span></td><td class="actions">${actions}</td></tr>`; }).join('') : '<tr><td class="empty-cell" colspan="6">No maintenance scheduled.</td></tr>';
}

function setForm(formId, values = {}) { const form = $(`#${formId}`); Object.entries(values).forEach(([key, value]) => { if (form.elements[key]) form.elements[key].value = value ?? ''; }); form.hidden = false; form.elements[formId === 'asset-form' ? 'tag_code' : 'task'].focus(); }
function resetForm(formId) { $(`#${formId}`).reset(); $(`#${formId}`).elements.id.value = ''; $(`#${formId}`).hidden = true; }

async function loadAll() {
  [state.options, state.assets, state.maintenance] = await Promise.all([request('/options'), request('/assets'), request('/maintenance')]);
...
document.addEventListener('click', async (event) => {
  ...
  try {
    if (button.dataset.editAsset) setForm('asset-form', state.assets.find((asset) => asset.id === Number(button.dataset.editAsset)));
    ...
    if (button.dataset.completeMaintenance && window.confirm('Mark this maintenance as completed?')) { await request(`/maintenance/${button.dataset.completeMaintenance}/complete`, { method: 'POST', body: JSON.stringify({}) }); await loadAll(); }
  } catch (error) { showError(error); }
});
