...
const auditRoles = ['admin', 'manager'];
...
async function auditEvent(assetId, action, details, actor) { await pool.execute('INSERT INTO audit_logs (entity_type, entity_id, action, details, actor) VALUES (?, ?, ?, ?, ?)', ['asset', assetId, action, JSON.stringify(details || {}), actor]); }
...
async function authenticate(request, response, next) {
  try {
    ...
    const [rows] = await pool.query(`SELECT s.id session_id, s.expires_at, u.id, u.full_name, u.email, u.role, u.approval_status, u.is_active
      FROM auth_sessions s JOIN users u ON u.id = s.user_id
      WHERE s.token_hash = ? AND s.revoked_at IS NULL AND s.expires_at > NOW() AND u.approval_status = 'approved' AND u.is_active = 1`, [tokenHash(token)]);
...
function redactStaffAssets(rows, request) { return request.user.role === 'staff' ? rows.map(({ purchase_cost, purchase_price, current_value, salvage_value, useful_life_years, ...asset }) => asset) : rows; }
function staffAssetFilter(request, alias = 'a') { return request.user.role === 'staff' ? { clause: ` AND ${alias}.assigned_to = ?`, values: [request.user.id] } : { clause: '', values: [] }; }
async function requireStaffAssetAccess(request, assetId) { if (request.user.role !== 'staff') return; const [[asset]] = await pool.query('SELECT id FROM assets WHERE id = ? AND assigned_to = ?', [assetId, request.user.id]); if (!asset) throw error('Asset not found', 404); }
...
app.post('/api/auth/register', optionalAuthenticate, asyncRoute(async (request, response) => {
  ...
  if (request.user && request.user.role !== 'admin') throw error('Only an admin can register users', 403);
  const [[count]] = await pool.query('SELECT COUNT(*) total FROM users WHERE password_hash IS NOT NULL');
  const initialAccount = Number(count.total) === 0;
  const requestedRole = initialAccount ? 'admin' : role(request.body.role);
  const approvalStatus = initialAccount || request.user ? 'approved' : 'pending';
  const [result] = await pool.execute('INSERT INTO users (full_name, email, password_hash, role, approval_status, is_active) VALUES (?, ?, ?, ?, ?, 1)', [fullName, userEmail, hashPassword(userPassword), requestedRole, approvalStatus]);
  response.status(approvalStatus === 'pending' ? 202 : 201).json({ id: result.insertId, full_name: fullName, email: userEmail, role: requestedRole, approval_status: approvalStatus });
}));
app.get('/api/users', authenticate, requireRole('admin'), asyncRoute(async (request, response) => {
  const [rows] = await pool.query('SELECT id, full_name, email, role, approval_status, is_active, created_at FROM users ORDER BY full_name'); response.json(rows);
}));
app.patch('/api/users/:id', authenticate, requireRole('admin'), asyncRoute(async (request, response) => {
  const userId = id(request.params.id, 'id'); const fullName = text(request.body.full_name, 'full_name', 120); const userEmail = email(request.body.email); const userRole = role(request.body.role);
  const [result] = await pool.execute('UPDATE users SET full_name = ?, email = ?, role = ? WHERE id = ?', [fullName, userEmail, userRole, userId]);
  if (!result.affectedRows) throw error('User not found', 404);
  response.json({ id: userId, full_name: fullName, email: userEmail, role: userRole });
}));
app.patch('/api/users/:id/status', authenticate, requireRole('admin'), asyncRoute(async (request, response) => {
  const userId = id(request.params.id, 'id'); if (userId === request.user.id && request.body.is_active === false) throw error('You cannot deactivate your own account', 400);
  const isActive = request.body.is_active === true ? 1 : request.body.is_active === false ? 0 : null; if (isActive === null) throw error('is_active must be true or false');
  const [result] = await pool.execute('UPDATE users SET is_active = ? WHERE id = ?', [isActive, userId]); if (!result.affectedRows) throw error('User not found', 404); response.json({ id: userId, is_active: Boolean(isActive) });
}));
app.post('/api/users/:id/approve', authenticate, requireRole('admin'), asyncRoute(async (request, response) => {
  const userId = id(request.params.id, 'id'); const [result] = await pool.execute("UPDATE users SET approval_status = 'approved', is_active = 1 WHERE id = ?", [userId]); if (!result.affectedRows) throw error('User not found', 404); response.status(204).end();
}));
app.post('/api/users/:id/reject', authenticate, requireRole('admin'), asyncRoute(async (request, response) => {
  const userId = id(request.params.id, 'id'); const [result] = await pool.execute("UPDATE users SET approval_status = 'rejected', is_active = 0 WHERE id = ?", [userId]); if (!result.affectedRows) throw error('User not found', 404); response.status(204).end();
}));
app.post('/api/auth/login', asyncRoute(async (request, response) => {
  const userEmail = email(request.body.email); const userPassword = password(request.body.password);
  const [rows] = await pool.query("SELECT id, full_name, email, password_hash, role FROM users WHERE email = ? AND approval_status = 'approved' AND is_active = 1", [userEmail]);
...
app.get('/api/options', asyncRoute(async (request, response) => {
  ...
  const [users] = request.user.role === 'staff' ? [[]] : await pool.query("SELECT id, full_name name, role FROM users WHERE role IN ('staff', 'manager') AND is_active = 1 AND approval_status = 'approved' ORDER BY full_name");
  response.json({ categories, locations, users });
}));
app.get('/api/dashboard', asyncRoute(async (request, response) => {
  try {
    const assetScope = request.user.role === 'staff' ? ' WHERE assigned_to = ?' : '';
    const scopeValues = request.user.role === 'staff' ? [request.user.id] : [];
    const [[summary]] = await pool.query(`SELECT COUNT(*) total_assets, COALESCE(SUM(condition_status = 'working'), 0) working,
      ...
      COALESCE(SUM(current_value), 0) book_value FROM assets${assetScope}`, scopeValues);
    const maintenanceScope = request.user.role === 'staff' ? ' AND assigned_to = ?' : '';
    const [[due]] = await pool.query(`SELECT COUNT(*) maintenance_due FROM maintenance_records WHERE status IN ('scheduled', 'in_progress') AND scheduled_date <= DATE_ADD(CURDATE(), INTERVAL 30 DAY)${maintenanceScope}`, scopeValues);
    ...
    const [upcomingMaintenance] = await pool.query(`SELECT m.id, a.tag_code, a.name asset_name, m.task, m.scheduled_date, m.status, m.approval_status FROM maintenance_records m JOIN assets a ON a.id = m.asset_id WHERE m.status IN ('scheduled', 'in_progress')${maintenanceScope} ORDER BY m.scheduled_date LIMIT 8`, scopeValues);
    response.json({ summary: { ...summary, ...due, book_value: request.user.role === 'staff' ? null : summary.book_value }, recentActivity: request.user.role === 'staff' ? [] : recentActivity, upcomingMaintenance, permissions: { canSeeFinancials: request.user.role !== 'staff', canSeeAudit: auditRoles.includes(request.user.role), canApprove: ['admin', 'manager'].includes(request.user.role), canWrite: ['admin', 'manager', 'staff'].includes(request.user.role), isStaff: request.user.role === 'staff' } });
  } catch (error) {
    console.error('Dashboard route failed', { code: error.code, message: error.message, sqlMessage: error.sqlMessage, userId: request.user?.id, role: request.user?.role });
    throw error;
  }
}));
  ...
  app.get('/api/assets', asyncRoute(async (request, response) => { const filter = staffAssetFilter(request); const [rows] = await pool.query(`${enhancedAssetSelect} WHERE 1=1${filter.clause} ORDER BY a.created_at DESC`, filter.values); response.json(redactStaffAssets(rows, request)); }));
  app.get('/api/assets/:id(\\d+)', asyncRoute(async (request, response) => { const assetId = id(request.params.id, 'id'); const filter = staffAssetFilter(request); const [rows] = await pool.query(`${enhancedAssetSelect} WHERE a.id = ?${filter.clause}`, [assetId, ...filter.values]); if (!rows[0]) throw error('Asset not found', 404); response.json(redactStaffAssets(rows, request)[0]); }));
  app.post('/api/assets', requireRole('admin'), asyncRoute(async (request, response) => {
    ...
    const name = text(body.name, 'name', 160); const serialNumber = text(body.serial_number, 'serial_number', 160); const supplier = text(body.supplier, 'supplier', 160); const purchasePrice = money(body.purchase_price ?? body.purchase_cost, 'purchase_price'); const salvageValue = money(body.salvage_value, 'salvage_value'); const usefulLife = Number(body.useful_life_years); if (!Number.isFinite(usefulLife) || usefulLife <= 0 || usefulLife > 100) throw error('useful_life_years must be between 0 and 100');
    const locationId = id(body.location_id, 'location_id'); const assignedTo = id(body.assigned_to, 'assigned_to'); const status = assetStatus(body.status || body.condition_status); const purchaseDate = dateValue(body.purchase_date, 'purchase_date'); const connection = await pool.getConnection();
    try { await connection.beginTransaction(); const tag = await uniqueTag(categoryPrefix, connection); const [result] = await connection.execute(`INSERT INTO assets (tag, tag_code, name, description, category_id, serial_number, purchase_date, purchase_cost, purchase_price, supplier, location_id, assigned_to, condition_status, status, useful_life_years, salvage_value, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [tag, tag, name, body.description || null, category.id, serialNumber, purchaseDate, purchasePrice, purchasePrice, supplier, locationId, assignedTo, status === 'repair' ? 'under_repair' : status, status, usefulLife, salvageValue, request.user.id]); await connection.execute('INSERT INTO location_history (asset_id, location, assigned_to, changed_by) VALUES (?, ?, ?, ?)', [result.insertId, body.location || null, assignedTo, request.user.id]); await connection.commit(); await auditEvent(result.insertId, 'registered', { tag, name }, request.user.id); const [rows] = await pool.query(`${enhancedAssetSelect} WHERE a.id = ?`, [result.insertId]); response.status(201).json({ ...rows[0], qr_value: `ASSETTRACK:${tag}`, qr_code: await QRCode.toDataURL(`ASSETTRACK:${tag}`) }); } catch (err) { await connection.rollback(); throw err; } finally { connection.release(); }
  }));
  app.get('/api/assets/:id/qr', asyncRoute(async (request, response) => { const assetId = id(request.params.id, 'id'); await requireStaffAssetAccess(request, assetId); const [[asset]] = await pool.query('SELECT tag FROM assets WHERE id = ?', [assetId]); if (!asset) throw error('Asset not found', 404); response.type('image/png').send(await QRCode.toBuffer(`ASSETTRACK:${asset.tag}`)); }));
  app.patch('/api/assets/:id/location', requireRole('admin', 'manager', 'staff'), asyncRoute(async (request, response) => { const assetId = id(request.params.id, 'id'); await requireStaffAssetAccess(request, assetId); const locationId = id(request.body.location_id, 'location_id'); const assignedTo = request.user.role === 'staff' ? request.user.id : id(request.body.assigned_to, 'assigned_to'); const location = request.body.location ? text(request.body.location, 'location', 160) : null; const connection = await pool.getConnection(); try { await connection.beginTransaction(); const [result] = await connection.execute('UPDATE assets SET location_id = ?, assigned_to = ? WHERE id = ?', [locationId, assignedTo, assetId]); if (!result.affectedRows) throw error('Asset not found', 404); await connection.execute('INSERT INTO location_history (asset_id, location, assigned_to, changed_by) VALUES (?, ?, ?, ?)', [assetId, location, assignedTo, request.user.id]); await connection.commit(); await auditEvent(assetId, 'location_changed', { location, assigned_to: assignedTo }, request.user.id); response.json({ ok: true }); } catch (err) { await connection.rollback(); throw err; } finally { connection.release(); } }));
  app.get('/api/assets/:id/history', asyncRoute(async (request, response) => { const assetId = id(request.params.id, 'id'); await requireStaffAssetAccess(request, assetId); const [rows] = await pool.query('SELECT h.*, u.full_name changed_by_name, a.full_name assigned_name FROM location_history h JOIN users u ON u.id = h.changed_by LEFT JOIN users a ON a.id = h.assigned_to WHERE h.asset_id = ? ORDER BY h.changed_at ASC', [assetId]); response.json(rows); }));
  app.patch('/api/assets/:id/status', requireRole('admin', 'manager', 'staff'), asyncRoute(async (request, response) => { const assetId = id(request.params.id, 'id'); await requireStaffAssetAccess(request, assetId); const status = assetStatus(request.body.status); if (status === 'retired' && request.user.role === 'staff') throw error('Only a manager or admin can retire an asset', 403); const [result] = await pool.execute("UPDATE assets SET status = ?, condition_status = CASE ? WHEN 'repair' THEN 'under_repair' ELSE ? END WHERE id = ?", [status, status, status, assetId]); if (!result.affectedRows) throw error('Asset not found', 404); await auditEvent(assetId, 'status_changed', { status }, request.user.id); response.json({ ok: true, status }); }));
  app.get('/api/assets/:id/depreciation', asyncRoute(async (request, response) => { if (request.user.role === 'staff') throw error('Financial details are restricted to management', 403); const assetId = id(request.params.id, 'id'); const [[asset]] = await pool.query('SELECT * FROM assets WHERE id = ?', [assetId]); if (!asset) throw error('Asset not found', 404); response.json(depreciation(asset)); }));

  app.get('/api/reports/summary', asyncRoute(async (request, response) => { if (request.user.role === 'staff') throw error('Financial reports are restricted to management', 403); const [[status]] = await pool.query("SELECT SUM(status = 'working') working, SUM(status = 'repair') repair, SUM(status = 'retired') retired, COUNT(*) total_assets FROM assets"); const [[value]] = await pool.query('SELECT COALESCE(SUM(GREATEST(salvage_value, purchase_price - ((purchase_price - salvage_value) / NULLIF(useful_life_years, 0)) * GREATEST(0, DATEDIFF(CURDATE(), purchase_date) / 365.25))), 0) total_book_value FROM assets'); const [[maintenance]] = await pool.query("SELECT SUM(status = 'scheduled' AND scheduled_date < CURDATE()) overdue, SUM(status = 'scheduled' AND scheduled_date >= CURDATE() AND scheduled_date <= DATE_ADD(CURDATE(), INTERVAL 30 DAY)) due_soon, SUM(status = 'completed') completed FROM maintenance_records"); const [categories] = await pool.query('SELECT c.name category, COUNT(*) count FROM assets a JOIN categories c ON c.id = a.category_id GROUP BY c.id, c.name ORDER BY count DESC'); const [locations] = await pool.query("SELECT COALESCE(l.name, 'Unassigned') location, COUNT(*) count FROM assets a LEFT JOIN locations l ON l.id = a.location_id GROUP BY l.id, l.name ORDER BY count DESC"); response.json({ status, total_book_value: value.total_book_value, maintenance, categories, locations }); }));

  app.get('/api/maintenance', asyncRoute(async (request, response) => { const filter = request.query.filter; const conditions = filter === 'completed' ? "m.status = 'completed'" : filter === 'overdue' ? "m.status = 'scheduled' AND m.scheduled_date < CURDATE()" : filter === 'upcoming' ? "m.status = 'scheduled' AND m.scheduled_date >= CURDATE()" : '1=1'; const assignment = request.user.role === 'staff' ? ' AND m.assigned_to = ?' : ''; const values = request.user.role === 'staff' ? [request.user.id] : []; const [rows] = await pool.query(`SELECT m.*, a.tag_code tag, a.name asset_name, u.full_name assigned_name FROM maintenance_records m JOIN assets a ON a.id = m.asset_id LEFT JOIN users u ON u.id = m.assigned_to WHERE ${conditions}${assignment} ORDER BY m.scheduled_date`, values); response.json(rows); }));
  app.post('/api/maintenance', requireRole('admin', 'manager', 'staff'), asyncRoute(async (request, response) => { const assetId = id(request.body.asset_id, 'asset_id'); const title = text(request.body.title || request.body.task, 'title', 255); const scheduledDate = dateValue(request.body.scheduled_date, 'scheduled_date'); let assignedTo = id(request.body.assigned_to, 'assigned_to'); if (request.user.role === 'staff') { await requireStaffAssetAccess(request, assetId); assignedTo = request.user.id; } const [result] = await pool.execute("INSERT INTO maintenance_records (asset_id, task, scheduled_date, assigned_to, status, approval_status) VALUES (?, ?, ?, ?, 'scheduled', 'pending')", [assetId, title, scheduledDate, assignedTo]); await auditEvent(assetId, 'maintenance_scheduled', { maintenance_id: result.insertId, title }, request.user.id); const [rows] = await pool.query('SELECT m.*, a.tag_code tag, a.name asset_name FROM maintenance_records m JOIN assets a ON a.id = m.asset_id WHERE m.id = ?', [result.insertId]); response.status(201).json(rows[0]); }));
  app.patch('/api/maintenance/:id/approval', requireRole('admin', 'manager'), asyncRoute(async (request, response) => { const maintenanceId = id(request.params.id, 'id'); const approvalStatus = String(request.body.approval_status || '').toLowerCase(); if (!['approved', 'rejected'].includes(approvalStatus)) throw error('approval_status must be approved or rejected'); const [result] = await pool.execute('UPDATE maintenance_records SET approval_status = ?, approved_by = ? WHERE id = ?', [approvalStatus, request.user.id, maintenanceId]); if (!result.affectedRows) throw error('Maintenance record not found', 404); response.json({ id: maintenanceId, approval_status: approvalStatus }); }));
  app.patch('/api/maintenance/:id/complete', requireRole('admin', 'staff'), asyncRoute(async (request, response) => { const maintenanceId = id(request.params.id, 'id'); const [rows] = await pool.query('SELECT asset_id, assigned_to FROM maintenance_records WHERE id = ?', [maintenanceId]); if (!rows[0]) throw error('Maintenance record not found', 404); if (request.user.role === 'staff' && rows[0].assigned_to !== request.user.id) throw error('You can only complete assigned maintenance', 403); const [result] = await pool.execute("UPDATE maintenance_records SET status = 'completed', completed_date = CURDATE() WHERE id = ?", [maintenanceId]); if (!result.affectedRows) throw error('Maintenance record not found', 404); await auditEvent(rows[0].asset_id, 'maintenance_completed', { maintenance_id: maintenanceId, notes: request.body.notes || null }, request.user.id); response.json({ ok: true }); }));
...
function assetPayload(body) {
  const condition = body.condition_status || 'working'; if (!validConditions.has(condition)) throw error('condition_status must be working, under_repair, or retired');
  return [text(body.tag_code, 'tag_code', 60), text(body.name, 'name', 160), body.description || null, id(body.category_id, 'category_id'), id(body.location_id, 'location_id'), id(body.assigned_to, 'assigned_to'), condition, dateValue(body.purchase_date, 'purchase_date'), money(body.purchase_cost, 'purchase_cost'), money(body.current_value, 'current_value'), text(body.serial_number, 'serial_number', 160), text(body.supplier, 'supplier', 160), Number(body.useful_life_years), money(body.salvage_value, 'salvage_value')];
}
app.post('/api/assets', requireRole('admin'), asyncRoute(async (request, response) => {
...
app.put('/api/assets/:id', requireRole('admin', 'manager', 'staff'), asyncRoute(async (request, response) => {
  if (request.user.role === 'staff' || request.user.role === 'manager') {
    const assetId = id(request.params.id, 'id'); const condition = request.body.condition_status === 'repair' ? 'under_repair' : request.body.condition_status || 'working';
    if (!validConditions.has(condition)) throw error('Invalid asset condition');
    if (condition === 'retired' && request.user.role === 'staff') throw error('Retirement requires manager or admin approval', 403);
    await requireStaffAssetAccess(request, assetId);
    const locationId = id(request.body.location_id, 'location_id'); const assignedTo = request.user.role === 'staff' ? request.user.id : id(request.body.assigned_to, 'assigned_to');
    const [result] = await pool.execute("UPDATE assets SET location_id=?, assigned_to=?, condition_status=?, status=CASE ? WHEN 'under_repair' THEN 'repair' WHEN 'retired' THEN 'retired' ELSE 'working' END WHERE id=?", [locationId, assignedTo, condition, condition, assetId]);
  ...
  if (!Number.isFinite(values[12]) || values[12] <= 0 || values[12] > 100) throw error('useful_life_years must be between 0 and 100');
  try { await connection.beginTransaction(); const [result] = await connection.execute('UPDATE assets SET tag_code=?, name=?, description=?, category_id=?, location_id=?, assigned_to=?, condition_status=?, purchase_date=?, purchase_cost=?, current_value=?, serial_number=?, supplier=?, useful_life_years=?, salvage_value=?, purchase_price=? WHERE id=?', [...values, values[8], assetId]); if (!result.affectedRows) throw error('Asset not found', 404); await audit(connection, 'asset', assetId, 'updated', { tag_code: values[0], name: values[1] }); await connection.commit(); const [rows] = await pool.query(`${assetSelect} WHERE a.id = ?`, [assetId]); response.json(rows[0]); } catch (err) { await connection.rollback(); throw err; } finally { connection.release(); }
}));
...
app.post('/api/maintenance', requireRole('admin', 'manager', 'staff'), asyncRoute(async (request, response) => { const values = maintenancePayload(request.body); if (!values[0]) throw error('asset_id is required'); if (request.user.role === 'staff') { await requireStaffAssetAccess(request, values[0]); values[3] = null; } const [result] = await pool.execute("INSERT INTO maintenance_records (asset_id, task, scheduled_date, completed_date, status, notes, assigned_to, approval_status) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending')", [...values, request.user.role === 'staff' ? request.user.id : id(request.body.assigned_to, 'assigned_to')]); await pool.execute('INSERT INTO audit_logs (entity_type, entity_id, action, details) VALUES (?, ?, ?, ?)', ['maintenance', result.insertId, 'created', JSON.stringify({ task: values[1] })]); const [rows] = await pool.query('SELECT m.*, a.tag_code, a.name asset_name FROM maintenance_records m JOIN assets a ON a.id=m.asset_id WHERE m.id=?', [result.insertId]); response.status(201).json(rows[0]); }));
